...
const SortPrimaryText = styled.button`
  cursor: pointer;
  font-weight: ${({ active, theme }) => (active ? 500 : 400)};
  margin-right: 0.75rem !important;
  border: none;
...
function TxnList({ transactions, symbol0Override, symbol1Override, color }) {
    ...
    return (
      <DashGrid style={{ height: '48px' }}>
        <DataText area="txn" fontWeight="500">
          <Link color={"#80bdff"} external href={urls.showTransaction(item.hash)}>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={"#80bdff"} external href={'https://explorer.zksync.io/address/' + item.account}>
  ...
  return (
    <>
      <DashGrid center={true} style={{ height: 'fit-content', padding: '0 1rem 1rem' }}>
        {below780 ? (
          <RowBetween area="txn">
