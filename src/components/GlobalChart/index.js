...
const GlobalChart = ({ display }) => {
  ...
  return chartDataFiltered ? (
    <>
      {below800 && (
        <DropdownSelect options={CHART_VIEW} active={chartView} setActive={setChartView} color={'#a0a0a0'} />
      )}
      ...
      {chartDataFiltered && chartView === CHART_VIEW.VOLUME && (
        <ResponsiveContainer aspect={60 / 28}>
          <TradingViewChart
            ...
            title={volumeWindow === VOLUME_WINDOW.DAYS ? 'volume 24h' : VOLUME_WINDOW.WEEKLY ? 'volume 7d' : 'volumne 1m'}
            field={volumeWindow === VOLUME_WINDOW.DAYS ? 'dailyVolumeUSD' : VOLUME_WINDOW.WEEKLY ? 'weeklyVolumeUSD' : 'monthlyVolumeUSD'}
            width={width}
