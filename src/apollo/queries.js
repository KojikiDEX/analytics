...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "studio/v94307") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const ETH_PRICE = (block) => {
  ...
  `
  //console.log("[===prince===]: getEthPrice: queryString", queryString)
  return gql(queryString)
}
...
export const PAIRS_HISTORICAL_BULK = (block, pairs) => {
  ...
  let queryString = `
  query pairs {
    pairs(first: 200, where: {id_in: ${pairsString}}, block: {number: ${block.number}}, orderBy: trackedReserveETH, orderDirection: desc) {
      id
      reserveUSD
      trackedReserveETH
      volumeUSD
