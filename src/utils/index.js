...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  return `https://app.kojiki.exchange/liquidity`
  // if (!token1Address) {
  //   return (
  //     `https://app.kojiki.exchange/liquidity` +
  //     (remove ? `remove` : `add`) +
  //     `/v2/${token0Address === '0x5aea5775959fbc2557cc8789bc1bf90a239d9a91' ? 'ETH' : token0Address}/${'ETH'}`
  //   )
  // } else {
  //   return (
  //     `https://app.kojiki.exchange/` +
  //     (remove ? `remove` : `add`) +
  //     `/v2/${token0Address === '0x5aea5775959fbc2557cc8789bc1bf90a239d9a91' ? 'ETH' : token0Address}/${token1Address === '0x5aea5775959fbc2557cc8789bc1bf90a239d9a91' ? 'ETH' : token1Address
  //     }`
  //   )
  // }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  return `https://app.kojiki.exchange/swap`
  // if (!token1Address) {
  //   return `https://app.kojiki.exchange/swap?inputCurrency=${token0Address}`
  // } else {
  //   return `https://app.kojiki.exchange/swap?inputCurrency=${token0Address === '0x5aea5775959fbc2557cc8789bc1bf90a239d9a91' ? 'ETH' : token0Address
  //     }&outputCurrency=${token1Address === '0x5aea5775959fbc2557cc8789bc1bf90a239d9a91' ? 'ETH' : token1Address}`
  // }
}

export function getMiningPoolLink(token0Address) {
  return `https://app.kojiki.exchange`
  // return `https://app.kojiki.exchange/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  return `https://app.kojiki.exchange`
  // let baseUniswapUrl = 'https://app.kojiki.exchange/uni'
  // if (!linkVariable) {
...
export function getTimestampsForChanges() {
  ...
  const t1 = utcCurrentTime.subtract(13, 'day').startOf('minute').unix()
  const t2 = utcCurrentTime.subtract(12, 'day').startOf('minute').unix()
  const tWeek = utcCurrentTime.subtract(2, 'week').startOf('minute').unix()
  return [t1, t2, tWeek]
}
...
export const urls = {
  showTransaction: (tx) => `https://explorer.zksync.io/tx/${tx}/`,
  showAddress: (address) => `https://www.explorer.zksync.io/address/${address}/`,
  showToken: (address) => `https://www.explorer.zksync.io/token/${address}/`,
  showBlock: (block) => `https://explorer.zksync.io/block/${block}/`,
}
